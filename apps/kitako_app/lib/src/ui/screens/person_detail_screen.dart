...
///
/// Shows the person's representative thumbnail, label/name,
/// total face count, and a grid of all images containing that person.
  ...
  Future<void> _loadPersonImages() async {
    ...
    try {
      ...
      for (final id in imageIds) {
        try {
          final img = await widget.searchService.imageLoader
              .getImageWithThumbnail(id);
          images.add(img);
        } catch (_) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            '${_person.faceCount} ${_person.faceCount == 1 ? 'appearance' : 'appearances'} '
            'in ${_images.length} ${_images.length == 1 ? 'photo' : 'photos'}',
            style: TextStyle(
              color: isDark ? Colors.white54 : Colors.black54,
            ),
          ),
          const SizedBox(height: 12),
          // Label button if not labeled
